...
def select_series_title_with_most_human_characters
  ...
  JOIN characters ON series.id = characters.series_id GROUP BY species ORDER BY COUNT(species) ASC LIMIT 1;"
end

def select_character_names_and_number_of_books_they_are_in
  "SELECT characters.name, COUNT(book_id) AS count FROM character_books
  JOIN characters ON character_books.character_id = characters.id
  GROUP BY character_id ORDER BY count DESC, characters.name"
end
